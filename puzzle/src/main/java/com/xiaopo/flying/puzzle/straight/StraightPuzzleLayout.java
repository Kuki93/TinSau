...
public abstract class StraightPuzzleLayout implements PuzzleLayout {
    ...
    public void sortAreas() {
        List<Float> standards = new ArrayList<>();
        standards.add(0.0f);
        boolean ok = true;
        for (StraightArea area : areas) {
            if (area.bottom() == 0.0f) {
                ok = false;
                break;
            } else {
     ...
     * @param standards
     */
    private void setAreasLevel(int level, List<Float> standards) {
        List<Float> newStandards = new ArrayList<>();
        boolean isContinue = false;
        for (StraightArea area : areas) {
            if (area.level == 0)
                for (Float standard : standards) {
                    if (area.top() == standard) {
                        area.level = level;
                        newStandards.add(area.bottom());
                        isContinue = true;
                        break;
                    }
                }
        ...
        if (isContinue)
            setAreasLevel(++level, newStandards);
    }
