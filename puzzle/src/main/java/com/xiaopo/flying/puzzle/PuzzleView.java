...
public class PuzzleView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // draw selected area
        if (handlingPiece != null && currentMode != ActionMode.SWAP && touchEnable) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (handlingPiece != null && onPieceSelectedListener != null && !isMoved) {
    ...
    public void setPiece(int position, Drawable drawable) {
        if (position >= puzzlePieces.size()) {
            addPiece(drawable);
            return;
        }
        PuzzlePiece piece = puzzlePieces.get(position);
        float scaleX = piece.getArea().getAreaRect().width() / drawable.getIntrinsicWidth();
        float scaleY = piece.getArea().getAreaRect().height() / drawable.getIntrinsicHeight();
        Matrix matrix = MatrixUtils.generateMatrix(piece.getArea(), drawable, 0f);
        piece.setDrawable(drawable);
        piece.set(matrix);
        invalidate();
    }
