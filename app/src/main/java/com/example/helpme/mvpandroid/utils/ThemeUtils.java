...
public class ThemeUtils {
    ...
    public static int getCurrentActionBarHeight(Context context) {
        TypedValue typedValue = new TypedValue();
        boolean isValue = context.getTheme().resolveAttribute(R.attr.actionBarSize, typedValue,
                true);
    ...
    public static Drawable setTintDrawable(Drawable drawable, Context context) {
        ...
        DrawableCompat.setTintList(drawable1, context.getResources().getColorStateList
                (getCurrentColorPrimary(context)));
        return drawable1;
    }
    
    public static Drawable setTintDrawable(@DrawableRes int drawable, Context context, @ColorRes
            int color) {
    ...
    public static int changeAlphaTo0(int color, float fraction) {
        int red = Color.red(color);
        int green = Color.green(color);
        int blue = Color.blue(color);
        int alpha = (int) (Color.alpha(color) * fraction);
        return Color.argb(Color.alpha(color) - alpha, red, green, blue);
    }
    
    public static int changeAlphaTo255(int color, float fraction) {
        int red = Color.red(color);
        int green = Color.green(color);
        int blue = Color.blue(color);
        int alpha = (int) (Color.alpha(color) * fraction);
